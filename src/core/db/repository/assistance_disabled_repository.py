...
class AssistanceCRUD(CRUDBase):
    async def get_full_address_by_telegram_id(self, telegram_id: int, session: AsyncSession):
        full_address = await session.execute(
            select(Assistance_disabled.full_address).where(Assistance_disabled.telegram_id == telegram_id)
        )
