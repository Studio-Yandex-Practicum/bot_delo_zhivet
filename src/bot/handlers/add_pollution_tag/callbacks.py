...
from src.bot.handlers.state_constants import (
    ADD_POLLUTION_TAG, BACK, CURRENT_FEATURE, FEATURES, NO_TAG, SAVE_TAG,
    TAG_ID,
)
from src.bot.service.tags import (
    check_pollution_tag_exists, get_all_pollution_tags,
)


async def input_pollution_tag(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    """Выбор тега из списка тегов"""
    buttons = []
    text = "Выберите один, наиболее подходящий тег:"
    tags = await get_all_pollution_tags()
    # для каждого тега из списка сделаем кнопку
    for tag in tags:
...
async def save_pollution_tag_to_context(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    """Сохранение тега."""
    if context.user_data[CURRENT_FEATURE] == ADD_POLLUTION_TAG:
        if update.callback_query.data == NO_TAG:
            context.user_data[FEATURES].pop(TAG_ID, None)
            return await back_to_select_option_to_report_about_pollution(update, context)
        if update.callback_query.data == BACK:
            return await back_to_select_option_to_report_about_pollution(update, context)
        if await check_pollution_tag_exists(update.callback_query.data):
    ...
    buttons = [
        [
            InlineKeyboardButton(text="Перейти к добавлению тега", callback_data=ADD_POLLUTION_TAG),
        ],
        [
