...
class MyAdminIndexView(AdminIndexView):
    ...
    def login_view(self):
        ...
        link = "<p>У Вас нет учетной записи? <a href=\"" + url_for(".register_view") + '">Нажмите здесь, чтобы зарегистрироваться</a></p>'
...
class StaffModelView(SuperuserModelView):
    ...
    form_columns = ("login", "roles", "active",)
...
class UserModelView(SuperuserModelView):
    ...
    column_labels = get_translated_lables(all_columns)
    form_columns = ("telegram_username", "is_banned",)
...
class VolunteerModelView(BaseModelView):    
    ...
    form_columns = (
        ...
        "full_address", "birthday", "is_banned",
    )
    column_labels = get_translated_lables(all_columns)
    form_widget_args = get_readonly_dict((
        "telegram_username", "first_name", "last_name", "full_address",
    ))
    ...
    def is_accessible(self):
        return (
            current_user.is_active and current_user.is_authenticated and
