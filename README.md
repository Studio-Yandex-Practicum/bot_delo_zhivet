...
1. В корне проекта лежат файлы `.env.X.example` с константами, которые необходимо заполнить тестовыми значениями. Удалите из имени файлов `.example` после окончания их настройки.
